...
public class ProjectMemberController {
    ...
    @GetMapping
    public ResponseEntity<List<MemberDetailResponse>> getMembers(@AuthenticationPrincipal UserModel user, @RequestBody GetMembersRequest request) {
        return ResponseEntity.ok(projectMemberService.getMembersByProjectId(user.getId(), request));
    }

    @PutMapping
    public ResponseEntity<Void> changeRole(@AuthenticationPrincipal UserModel user, @RequestBody ChangeRoleRequest request) {
        projectMemberService.changeRole(user.getId(), request);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @DeleteMapping
    public ResponseEntity<Void> deleteMember(@AuthenticationPrincipal UserModel user, @RequestBody DeleteMemberRequest request) {
